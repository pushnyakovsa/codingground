...
namespace task_ab
{
    public class ab
    {
        public long get(int a, int b)
        {
            ...
            char[] txt_array =  new char[len];

            for (int i = 3; i < a; i++)
            {
                for (int j = 3; j < b; j++)
                {
                    txt_array[len] = Convert.ToChar((long)Math.Pow(a, b));
                    len = len - 1;
                }
            }
            
            Array.Sort(txt_array);
            
            int_array = Array.ConvertAll(txt_array, c => (int)Char.GetNumericValue(c));
